...
static Hyprlang::CParseResult handleWallpaper(const char* C, const char* V) {
    ...
    if (!std::filesystem::exists(WALLPAPER, ec)) {
        const auto errorMsg = ec ? ec.message() : std::string{"no such file"};
        result.setError((std::string{"wallpaper failed ("} + errorMsg + std::string{": "} + WALLPAPER + std::string{")")).c_str());
        return result;
    }

    // Handle directory wallpaper selection
    if (std::filesystem::is_directory(WALLPAPER)) {
        std::string currentWallpaper;
        if (g_pHyprpaper->m_mMonitorActiveWallpapers.contains(MONITOR)) {
            currentWallpaper = g_pHyprpaper->m_mMonitorActiveWallpapers[MONITOR];
        }
        
        std::string randomImage = getRandomImageFromDirectory(WALLPAPER, currentWallpaper);
        if (randomImage.empty()) {
            result.setError("wallpaper failed (no valid images in directory)");
            return result;
        }
